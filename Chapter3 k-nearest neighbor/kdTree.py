...
class kdTree:
    ...
    def nearest(self,n1,n2,sample):
        if n1 is None:
            return n2,np.linalg.norm(n2.sample[:-1] - sample,ord=2)
        if n2 is None:
            return n1,np.linalg.norm(n1.sample[:-1] - sample,ord=2)
        d1,d2 = np.linalg.norm(n1.sample[:-1] - sample,ord=2),np.linalg.norm(n2.sample[:-1] - sample,ord=2)
        if  d1 < d2:
            return n1,d1
        else:
            return n2,d2
    ...
    def knn(self,k):
        self.k_min = [None for i in range(k)]

    def nearestNeighbor(self,root,sample,dim): # sample:(features,) without any labeling!
        if root is None:
            return root
...
if __name__ == '__main__':
    ...
    dataset.plotDataset(train_data)
